...
exports.getBalanceAndLoanAmountByFilterService = async (req) => {
  try {
    let getDataformGetBalanceAndLoanAmountByFilter =
      await Customertxn_Repo.getBalanceAndLoanAmountByFilter(req);
    logger.info(
      `get the data form Repo  of getBalanceAndLoanAmountByFilter ${getDataformGetBalanceAndLoanAmountByFilter} `
    );

    return getDataformGetBalanceAndLoanAmountByFilter;
  } catch (err) {
    logger.error(
      "err in the service getBalanceAndLoanAmountByFilterService",
      err
    );
    throw err;
  }
...
exports.getcCustomerTxnByNameService = async (req) => {
  try {
    let getcCustomerTxnByCustIdServiceData =
      await Customertxn_Repo.getcCustomerTxnByName(req);
    logger.info(
      `get the data form Repo  of getcCustomerTxnByCustIdServiceData ${getcCustomerTxnByCustIdServiceData} `
    );
...
exports.getSavingDetailsByFilterService = async (req) => {
  try {
    let getSavingDataFilter = await Customertxn_Repo.getSavingDetailsByFilter(
      req
    );
    logger.info(
